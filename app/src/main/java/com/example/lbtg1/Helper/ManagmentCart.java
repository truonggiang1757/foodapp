...
public class ManagmentCart {
    ...
    public ArrayList<Order> convertCartToOrder() {
        ArrayList<Foods> listItem = getListCart();
        ArrayList<Order> orders = new ArrayList<>();
        for (Foods food : listItem) {
            Order order = new Order();
            // Assuming Order class has necessary setters
            order.setOrderTitle(food.getTitle()); // Set order title as food name
            order.setStatus("Pending"); // Set initial status as Pending
            order.setTotalPrice(food.getPrice() * food.getNumberInCart()); // Calculate total price
            // You might need to set other properties of the order as well
            orders.add(order);
        }
